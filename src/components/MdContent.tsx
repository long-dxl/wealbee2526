...
export function MdContent({ text, refs }: { text: string; refs?: RefEntry[] }) {
  ...
  while (i < lines.length) {
    ...
    if (trim.startsWith("### ") && !trim.startsWith("#### ")) {
      nodes.push(<h4 key={i} style={{ margin: "12px 0 5px", fontSize: "0.875rem", fontWeight: 700, color: "#374151", fontFamily: "'Montserrat',sans-serif", borderLeft: "3px solid rgba(8,73,172,0.18)", paddingLeft: 8 }}>{trim.slice(4)}</h4>); i++; continue;
    }
    if (trim.startsWith("#### ")) {
